...
async function handleBriefing(request, env) {
  ...
  try {
    const routeUrl = `https://api.tomtom.com/routing/1/calculateRoute/${origin.lat},${origin.lon}:${destination.lat},${destination.lon}/json?key=${TOMTOM_KEY}&routeType=fastest&traffic=true&instructionsType=text&language=es-CL&computeBestOrder=false`;
    ...
    const rawInstructions = (route.guidance && route.guidance.instructions) || [];
    const guidance = rawInstructions.map((ins) => ({
      lat: ins.point ? ins.point.latitude : null,
      lon: ins.point ? ins.point.longitude : null,
      distanceMeters: ins.routeOffsetInMeters || 0,
      message: ins.message || ins.street || "Continúa recto",
      street: ins.street || "",
    ...
    return jsonResponse({
      ...
      ruta: { puntos: points.map((p) => [p.latitude, p.longitude]), guidance },
      clima: {
